...
-- 이름, 성별, 입사일 순서이고 “이름”, “성별”, “입사일"로 컬럼 이름을 대체해 보세요.
...
-- 1.
select count(distinct emp_no) as 'moreThan120,000'
 from salaries where salary >= 120000;
-- 2.
select count(emp.emp_no) as 'moreThan120,000'
  from (select emp_no
		from salaries
		where salary >= 120000
...
-- 1. 
select count(distinct emp_no) as EnginnerTotal
from titles where title = 'Engineer';
 
-- 2. 
select  count(*) as EnginnerTotal
from (
select count(*) as cnt
  from titles
